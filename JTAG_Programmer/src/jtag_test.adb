...
procedure jtag_test is
   ...
   begin
      ...
      DMA1_Periph.CCR3 :=
        (MINC => 1, CIRC => 1, PL => 2, EN => 1, others => <>);

      --  USART2 Configuration (2,000,000 Baud @ 48MHz)
      USART2_Periph.BRR :=
        (DIV_Mantissa => 16#01#, DIV_Fraction => 16#08#, others => <>);
   ...
   begin
      ...
      for I in 0 .. 7 loop
         ...
         if I = 7 then
            Pin_High
              (tms_pin); -- Pull TMS high on the last bit to exit Shift-IR
   ...
   begin
      ...
      cmd := (1, 0, 0, 0, 0, 0, 1, 0); --  read Status Register (IR=0x41)
      ...
      cmd := (1, 0, 1, 0, 1, 0, 0, 0); --  command (IR=0x15)
      Send_Command (cmd);

      cmd := (1, 0, 0, 0, 0, 0, 1, 0); --  command (IR=0x41)
      Send_Command (cmd);
      ...
      cmd := (1, 0, 1, 0, 0, 0, 0, 0); --  command (IR=0x05)
      Send_Command (cmd);
      cmd := (0, 1, 0, 0, 0, 0, 0, 0); --  command (IR=0x02)
      Send_Command (cmd);
      cmd := (1, 0, 0, 0, 0, 0, 1, 0); --  command (IR=0x41)
      Send_Command (cmd);
      Read_TDO; -- Read TDO for staus register
      cmd := (1, 0, 0, 1, 0, 0, 0, 0); --  command (IR=0x09)
      Send_Command (cmd);
      cmd := (0, 1, 0, 0, 0, 0, 0, 0); --  command (IR=0x02)
      Send_Command (cmd);
      cmd := (0, 1, 0, 1, 1, 1, 0, 0); --  command (IR=0x3A)
      Send_Command (cmd);
      cmd := (0, 1, 0, 0, 0, 0, 0, 0); --  command (IR=0x02)
      Send_Command (cmd);
      cmd := (1, 0, 0, 0, 0, 0, 1, 0); --  command (IR=0x41)
      ...
      cmd := (1, 0, 1, 0, 1, 0, 0, 0); --  command (IR=0x15)
      Send_Command (cmd);
      cmd := (0, 1, 0, 0, 1, 0, 0, 0); --  command (IR=0x12)
      Send_Command (cmd);
      cmd := (1, 1, 1, 0, 1, 0, 0, 0); --  command (IR=0x17)
      Send_Command (cmd);
...
begin
   --  TODO: Add UART command handling to trigger different JTAG operations
   Initialize_Hardware;
   Reset_TAP;
   Init_Configuration;
